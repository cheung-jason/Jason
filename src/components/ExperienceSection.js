...
function ExperienceSection({ experiences }) {
  ...
  return (
    <Box sx={{ minHeight: '40vh', backgroundColor: '#0a192f', color: '#e6f1ff', display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'flex-start', justifyContent: 'center', py: { xs: 6, md: 10 }, gap: 6 }}>
      ...
      <Box sx={{ flex: 1, ml: { md: 6 }, mt: { xs: 4, md: 0 }, p: 0, minWidth: 0 }}>
        <FadeInSection delay={0}>
          <Typography variant="h5" sx={{ fontWeight: 700, mb: 1 }}>
            <span style={{ color: '#a8b2d1' }}>{experiences[selected].title} @ </span>
            <span style={{ color: '#90a4ae' }}>{experiences[selected].company.split('–')[0]}</span>
          </Typography>
        </FadeInSection>
